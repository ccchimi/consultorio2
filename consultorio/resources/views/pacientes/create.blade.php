        ...
        <form action="{{ route('pacientes.store') }}" method="POST" class="space-y-4">
            ...
            <div x-show="tab === 'datos'" x-transition>
                <div>
                    ...
                    <input type="text" name="nombre" class="w-full px-4 py-2 bg-white/70 text-black border border-gray-300 rounded-md focus:ring-indigo-500 focus:outline-none" value="{{ old('nombre') }}" required>
                    @error('nombre') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-white mb-1">DNI (sin puntos)</label>
                    <input type="text" name="dni" class="w-full px-4 py-2 bg-white/70 text-black border border-gray-300 rounded-md focus:ring-indigo-500 focus:outline-none" value="{{ old('dni') }}" required>
            ...
            <div x-show="tab === 'contacto'" x-transition>
                <div>
                    ...
                    <input type="email" name="email" class="w-full px-4 py-2 bg-white/70 text-black border border-gray-300 rounded-md focus:ring-indigo-500 focus:outline-none" value="{{ old('email') }}" required>
                    @error('email') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
                </div>
                <div>
                    <label class="block text-sm font-semibold text-white mb-1">Teléfono</label>
                    <input type="text" name="telefono" class="w-full px-4 py-2 bg-white/70 text-black border border-gray-300 rounded-md focus:ring-indigo-500 focus:outline-none" value="{{ old('telefono') }}" required>
                ...
                <div>
                    ...
                    <input type="date" name="fecha_nacimiento" class="w-full px-4 py-2 bg-white/70 text-black border border-gray-300 rounded-md focus:ring-indigo-500 focus:outline-none" value="{{ old('fecha_nacimiento') }}" required>
